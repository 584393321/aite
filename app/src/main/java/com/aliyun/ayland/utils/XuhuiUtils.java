...
import static com.aliyun.ayland.utils.ResourceUtils.getString;


public final class XuhuiUtils {
    ...
    public static void restartSystem() {
        ATApplication.getContext().sendBroadcast(new Intent("atte.restart.system"));
        PackageManager pm = ATApplication.getContext().getPackageManager();
        boolean bAccess = (PackageManager.PERMISSION_GRANTED ==
                pm.checkPermission("android.permission.REBOOT", ATApplication.getContext().getPackageName()));
        if (bAccess) {
            PowerManager pManager = (PowerManager) ATApplication.getContext().getSystemService(Context.POWER_SERVICE);  //重启到fastboot模式
            pManager.reboot("");
        } else {
            ToastUtils.shortShow(getString(R.string.at_have_no_permission));
        }
    }
