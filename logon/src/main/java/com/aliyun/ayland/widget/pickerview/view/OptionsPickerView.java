...
public class OptionsPickerView<T> extends BasePickerView implements View.OnClickListener {
    ...
    private void initView(Context context) {
        ...
        mWheelOptions = new WheelOptions(optionsPicker, mPickerOptions.isRestoreItem);
        if (mPickerOptions.optionsSelectChangeListener != null) {
            mWheelOptions.setOptionsSelectChangeListener(mPickerOptions.optionsSelectChangeListener);
        }

        mWheelOptions.setTextContentSize(mPickerOptions.textSizeContent);
        mWheelOptions.setLabels(mPickerOptions.label1, mPickerOptions.label2, mPickerOptions.label3);
        mWheelOptions.setTextXOffset(mPickerOptions.x_offset_one, mPickerOptions.x_offset_two, mPickerOptions.x_offset_three);
        mWheelOptions.setCyclic(mPickerOptions.cyclic1, mPickerOptions.cyclic2, mPickerOptions.cyclic3);
        mWheelOptions.setTypeface(mPickerOptions.font);

        setOutSideCancelable(mPickerOptions.cancelable);

        mWheelOptions.setDividerColor(mPickerOptions.dividerColor);
        mWheelOptions.setDividerType(mPickerOptions.dividerType);
        mWheelOptions.setLineSpacingMultiplier(mPickerOptions.lineSpacingMultiplier);
        mWheelOptions.setTextColorOut(mPickerOptions.textColorOut);
        mWheelOptions.setTextColorCenter(mPickerOptions.textColorCenter);
        mWheelOptions.isCenterLabel(mPickerOptions.isCenterLabel);
    }
    ...
    private void reSetCurrentItems() {
        if (mWheelOptions != null) {
            mWheelOptions.setCurrentItems(mPickerOptions.option1, mPickerOptions.option2, mPickerOptions.option3);
        }
    }
    ...
    public void setNPicker(List<T> options1Items,
        ...
        mWheelOptions.setLinkage(false);
        mWheelOptions.setNPicker(options1Items, options2Items, options3Items);
        reSetCurrentItems();
    }
