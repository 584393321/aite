...
public class ATKeyboardPatch {
    ...
    private ATBarParams mBarParams;
    ...
    private ATKeyboardPatch(Activity activity) {
        this(activity, ((FrameLayout) activity.getWindow().getDecorView().findViewById(android.R.id.content)).getChildAt(0));
    }

    private ATKeyboardPatch(Activity activity, View contentView) {
        this(activity, null, "", contentView);
    }

    private ATKeyboardPatch(Activity activity, Dialog dialog, String tag) {
        this(activity, dialog, tag, dialog.getWindow().findViewById(android.R.id.content));
    }

    private ATKeyboardPatch(Activity activity, Dialog dialog, String tag, View contentView) {
        ...
        this.mBarParams = dialog != null ? ATImmersionBar.with(activity, dialog, tag).getBarParams()
                : ATImmersionBar.with(activity).getBarParams();
        if (mBarParams == null)
            throw new IllegalArgumentException("先使用ImmersionBar初始化");
    }

    private ATKeyboardPatch(Activity activity, Window window) {
        this.mActivity = activity;
        this.mWindow = window;
        this.mDecorView = mWindow.getDecorView();
        ...
        ATBarConfig barConfig = new ATBarConfig(mActivity);
    ...
    public static ATKeyboardPatch patch(Activity activity) {
        return new ATKeyboardPatch(activity);
    }

    public static ATKeyboardPatch patch(Activity activity, View contentView) {
        return new ATKeyboardPatch(activity, contentView);
    }

    public static ATKeyboardPatch patch(Activity activity, Dialog dialog, String tag) {
        return new ATKeyboardPatch(activity, dialog, tag);
    }

    public static ATKeyboardPatch patch(Activity activity, Dialog dialog, String tag, View contentView) {
        return new ATKeyboardPatch(activity, dialog, tag, contentView);
    }

    protected static ATKeyboardPatch patch(Activity activity, Window window) {
        return new ATKeyboardPatch(activity, window);
    }

    protected void setBarParams(ATBarParams barParams) {
