...
import com.aliyun.ayland.widget.pickerview.configure.PickerOptions;
import com.aliyun.ayland.widget.pickerview.listener.ISelectTimeCallback;
...
public class TimePickerView extends BasePickerView implements View.OnClickListener {
    ...
    private WheelTime mWheelTime; //自定义控件
    ...
    private void initWheelTime(LinearLayout timePickerView) {
        mWheelTime = new WheelTime(timePickerView, mPickerOptions.type, mPickerOptions.textGravity, mPickerOptions.textSizeContent);
        if (mPickerOptions.timeSelectChangeListener != null) {
            mWheelTime.setSelectChangeCallback(new ISelectTimeCallback() {
                @Override
        ...
        mWheelTime.setLunarMode(mPickerOptions.isLunarCalendar);
        ...
        setTime();
        mWheelTime.setLabels(mPickerOptions.label_year, mPickerOptions.label_month, mPickerOptions.label_day
                , mPickerOptions.label_hours, mPickerOptions.label_minutes, mPickerOptions.label_seconds);
        mWheelTime.setTextXOffset(mPickerOptions.x_offset_year, mPickerOptions.x_offset_month, mPickerOptions.x_offset_day,
                mPickerOptions.x_offset_hours, mPickerOptions.x_offset_minutes, mPickerOptions.x_offset_seconds);

        setOutSideCancelable(mPickerOptions.cancelable);
        mWheelTime.setCyclic(mPickerOptions.cyclic);
        mWheelTime.setDividerColor(mPickerOptions.dividerColor);
        mWheelTime.setDividerType(mPickerOptions.dividerType);
        mWheelTime.setLineSpacingMultiplier(mPickerOptions.lineSpacingMultiplier);
        mWheelTime.setTextColorOut(mPickerOptions.textColorOut);
        mWheelTime.setTextColorCenter(mPickerOptions.textColorCenter);
        mWheelTime.isCenterLabel(mPickerOptions.isCenterLabel);
    }
    ...
    private void setRange() {
        mWheelTime.setStartYear(mPickerOptions.startYear);
        mWheelTime.setEndYear(mPickerOptions.endYear);
    }
    ...
    private void setRangDate() {
        mWheelTime.setRangDate(mPickerOptions.startDate, mPickerOptions.endDate);
    ...
    private void setTime() {
        ...
        mWheelTime.setPicker(year, month, day, hours, minute, seconds);
    }
    ...
    public void setLunarCalendar(boolean lunar) {
        try {
            int year, month, day, hours, minute, seconds;
            Calendar calendar = Calendar.getInstance();
            calendar.setTime(WheelTime.dateFormat.parse(mWheelTime.getTime()));
            year = calendar.get(Calendar.YEAR);
            month = calendar.get(Calendar.MONTH);
            day = calendar.get(Calendar.DAY_OF_MONTH);
            hours = calendar.get(Calendar.HOUR_OF_DAY);
