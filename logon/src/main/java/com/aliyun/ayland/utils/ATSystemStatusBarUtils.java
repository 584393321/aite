...
import com.aliyun.ayland.widget.barlibrary.ATImmersionBar;
...
//                .fullScreen(false)      //有导航栏的情况下，activity全屏显示，也就是activity最下面被导航栏覆盖，不写默认非全屏
    ...
    public static ATImmersionBar getInstance(Activity activity) {
        return ATImmersionBar.with(activity);
    }
    ...
    public static void destroy(Activity activity) {
        if (Build.VERSION.SDK_INT < 21)
            return;
        ATImmersionBar.with(activity).destroy(); //不调用该方法，如果界面bar发生改变，在不关闭app的情况下，退出此界面再进入将记忆最后一次bar改变的状态
    }
